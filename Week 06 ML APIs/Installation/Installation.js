...
function setup() {
  // console.log(ageVector);
    ...
    flippedVideo = pg = createGraphics(100, 100);
    ...
    let currentVector = (v0.multiply(1 - positionBetweenVectors).add(v1.multiply(positionBetweenVectors))).tolist();  // createRandomVector();
...
function gotPos(poses) {
    ...
    if (thisNose.confidence > .70) {
        ...
        let handToNose =  dist(poses[0].pose.leftWrist.x,poses[0].pose.leftWrist.y, poses[0].pose.nose.x,poses[0].pose.nose.y);
        if (handToNose < 30)location.reload() ;
...
function draw() {
    ...
    let dWidth = 100; //Number: the width of the destination rectangle
    let dHeight = 100;//Number: the height of the destination rectangle
    let sx = poseResults.x - poseResults.radius * 2; //Number: the x-coordinate of the subsection of the source image to draw into the destination rectangle
    let sy = poseResults.y - poseResults.radius * 2; //Number: the y-coordinate of the subsection of the source image to draw into the destination rectangle
    let sWidth = poseResults.radius * 4; //Number: the width of the subsection of the source image to draw into the destination rectangle (Optional)
    let sHeight = poseResults.radius * 4; //Number: the height of the subsection of the source image to draw into the destination rectangle (Optional)
   
    flippedVideo.push();
    flippedVideo.translate(50, 50)
   
    flippedVideo.rotate(extraRotate);
    flippedVideo.translate(-50, -50)
    flippedVideo.translate(100, 0);
...
for(var i = 0; i < 18; i++){
        let ageVec   = nj.array(ageVector[i] );
...
class VectorPosition {
    ...
    drawIt() {
        ...
        if (this.stage == "ASKING") {
            if (this.firstTime) {
                ...
                text("Warming Up The GAN, Can Take A Few Minutes", this.x + (512 - tw) / 2, this.y + 200);
    ...
    loaded(person) {
        person.stage = "LOADED";
        person.myBlend = 0;
        this.fader = setInterval(function () { person.fadeIn(person); }, 5);
