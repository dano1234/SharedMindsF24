...
// Function to add DragControls event listeners
...
addDragControls(); // Initial call to setup event listeners
...
function addDuck() {
    loader.load('./duck/Duck.gltf', function (gltf) {
        ...
        duck.position.set(Math.random() * 20, Math.random() * 20, Math.random() * 20);
        scene.add(duck);
        draggableObjects.push(duck);
...
document.getElementById('addDuckButton').addEventListener('click', addDuck);
