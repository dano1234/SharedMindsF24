...
wsServer.on('request', function (request) {
    ...
    placeInCircle += 180 ;
    if (clients.length % 2 == 0)  placeInCircle += 180/clients.length 
    let p =  placeInCircle % 360

    connection.send(JSON.stringify({ type: 'initial', contents:{ "ID": ID, "placeInCircle":p} }));
  ...
  //  }
    ...
    connection.on('message', function (message) {
        ...
        }else if (message.type === 'utf8') { // accept only text and sends it around
