...
function setup() {
    ...
    talkToRunway("landmark",  startVector, 45);
    endVector = createRandomVector();
    talkToRunway("landmark",  endVector, 90);
}
function settled() {
    console.log("settled");
    ...
    let distanceBetweenMeAndLandmark0 = dist(people["me"].object.position.x, people["me"].object.position.y, people["me"].object.position.z, landmarks[1].object.position.x,landmarks[1].object.position.y,landmarks[1].object.position.z);
    let t = distanceBetweenMeAndLandmark0/distanceBetweenLandmarks;
    //let t = ((people["me"].angleOnCircle)/4)/Math.PI;
    console.log(t);
    let v1 = nj.array(startVector);
    let v0 = nj.array(endVector);
...
function talkToRunway(reason, vector, angle) {
    ...
    model.query(data).then(outputs => {
        ...
        let runway_img = createImg(image, "alter_ego","",
...
function computeCameraOrientation() {
    ...
    lon = Math.max(0, Math.min(45, lon));  //restrict movement
