...
import { initMoveCameraWithMouse, initHTML } from './interactionFrames.js';
...
function recall() {
    console.log("recall");
    FB.setFolder("/" + whichFrame + "/frameInfo/");
    FB.getStuffFromFirebase();
    // FB.subscribeToData(whichFrame + "/objects"); //get notified if anything changes in this folder
}
export function captureScene() {
    ...
    const frameData = {
        ...
        camera: {
            ...
            fov: camera.fov,
            target: camera.matrix.toArray()
        },
        objects: { hi: "ho" },//myObjectsByFirebaseKey,
        timestamp: Date.now()
    };
    FB.setFolder("/" + whichFrame + "/frameInfo/");
    FB.setDataInFirebase(frameData);

}



export function reconstitueObjects(incomingData) {
    for (let key in incomingData) {
        reactToFirebase("added", incomingData[key], key);
    }
}
